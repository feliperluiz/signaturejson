...
filename=$(basename "$1")
testcase="${filename%.*}"
echo -e "\e[32m>>>Running $testcase on Dinamo and Kryptus\e[0m"
echo

mkdir -p report

python3 xml_ktc_runner_dinamo_kryptus.py
...
sed -i -e "s/.\{100\}/&\n/g" report/diff.tex
sed "s/@testcase/$testcase/g" template_dinamo_kryptus.tex > report/report.tex

echo -e "\e[32m>>>Compiling report\e[0m"
pdflatex -output-directory report/ report/report
cp report/report.pdf .
mv report.pdf ktc_test_report_dinamo_kryptus.pdf
